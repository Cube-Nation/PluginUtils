...
public class BukkitCommand extends Command implements PluginIdentifiableCommand {
    ...
    protected BukkitCommand(ChatCommand newChatCommand) {
        super(newChatCommand.getMainAliases().get(0));

        this.executor = newChatCommand.getPlugin();
        this.owningPlugin = newChatCommand.getPlugin();
        this.usageMessage = "";

        if (!newChatCommand.getPermissions().isEmpty()) {
            setPermission(StringUtils.join(newChatCommand.getPermissions(), ";"));
        }

        if (newChatCommand.getMainAliases().size() > 1) {
            List<String> aliasList = new ArrayList<String>();

            Iterator<String> iterator = newChatCommand.getMainAliases().iterator();
            iterator.next();
            while (iterator.hasNext()) {
                aliasList.add(iterator.next());
            }
     ...
     *             if the completer or executor throw an exception during the
     *             process of tab-completing.
